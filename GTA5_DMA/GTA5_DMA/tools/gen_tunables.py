...
def render(bin_path, meta, rows):
    ...
    lines.append("")
    lines.append("\t// 运行期闸门与离线单测共用的判定：当前值是否是「游戏自己放回来的合法状态」")
    lines.append("\t//   · Value 型：落在登记区间内")
    lines.append("\t//   · Int/Float 型：等于表内已知默认值（换战局/刷新 tunables 时游戏会重置回默认）")
    lines.append("\t//   · 或等于该条登记的可接受替代值（altValue，如踢出计时的 INT_MAX 禁用态）")
    lines.append("\tinline bool IsLegitValue(const Entry& entry, int32_t bits)")
    lines.append("\t{")
    lines.append("\t\tif (entry.kind == Kind::Value)")
    lines.append("\t\t\treturn bits >= entry.minValue && bits <= entry.maxValue;")
    lines.append("\t\tif (bits == entry.expectedDefault)")
    lines.append("\t\t\treturn true;")
    lines.append("\t\tif (entry.altValue != 0 && bits == entry.altValue)")
    lines.append("\t\t\treturn true;")
